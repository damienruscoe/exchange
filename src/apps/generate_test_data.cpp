...
MarketCondition parse_market_condition(const std::string& s) {
    if (s == "HighVolatility") return MarketCondition::HighVolatility;
    if (s == "FlashCrash") return MarketCondition::FlashCrash;
    if (s == "BookChurn") return MarketCondition::BookChurn;
    if (s == "QuoteStuffing") return MarketCondition::QuoteStuffing;
    if (s == "LargeOrderImbalance") return MarketCondition::LargeOrderImbalance;
    if (s == "LiquidityDrain") return MarketCondition::LiquidityDrain;
    if (s == "PriceJump") return MarketCondition::PriceJump;
...
void generate_data(const std::string& output_filename, MarketCondition condition)
{
    const size_t num_messages = 500000; // Half a million messages
    const int64_t initial_mid_price = 100000000000; // e.g., $100.00 with 7 decimal places
    ...
    std::uniform_int_distribution<> action_dist(0, 100); // 0-100 for action probability
    std::uniform_int_distribution<> price_move_dist_gen(-10, 10); // Generator for mid price moves
    ...
    std::map<uint64_t, databento::MboMsg> active_orders; // To track orders for cancellations/modifications
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        bid_msg.hd.ts_event = std::chrono::system_clock::now(); // Assign time_point directly
        ...
        bid_msg.side = static_cast<databento::side::Side>('B'); // Corrected: static_cast to enum
        bid_msg.action = static_cast<databento::action::Action>('A'); // Corrected: static_cast to enum
        ...
        ask_msg.hd.ts_event = std::chrono::system_clock::now(); // Assign time_point directly
        ...
        ask_msg.side = static_cast<databento::side::Side>('A'); // Corrected: static_cast to enum
        ask_msg.action = static_cast<databento::action::Action>('A'); // Corrected: static_cast to enum
        encoder.EncodeRecord(ask_msg); // Pass only the record
        active_orders[ask_msg.order_id] = ask_msg;
    }

    // Parameters for FlashCrash
    const size_t flash_crash_start_message_idx = num_messages / 4; // Start 1/4 way through
    const size_t flash_crash_duration_messages = num_messages / 8; // Last for 1/8 of messages
    const int64_t flash_crash_price_drop_per_message = 5 * price_tick; // Drop 5 ticks per message
    ...
    for (size_t i = 0; i < num_messages; ++i) {
        ...
        msg.hd.ts_event = std::chrono::system_clock::now(); // Assign time_point directly
        ...
        switch (condition) {
            ...
            case MarketCondition::FlashCrash: {
                if (i >= flash_crash_start_message_idx && i < flash_crash_start_message_idx + flash_crash_duration_messages) {
            ...
            case MarketCondition::BookChurn: {
                ...
                current_add_prob = 100 - (book_churn_cancel_prob + book_churn_modify_prob); // Remaining for adds
            ...
            case MarketCondition::QuoteStuffing: {
                ...
                current_aggressive_prob = 100 - (quote_stuffing_add_prob + quote_stuffing_cancel_prob);
                current_min_qty = quote_stuffing_min_qty;
                current_max_qty = quote_stuffing_max_qty;
                break;
            }
            case MarketCondition::LargeOrderImbalance: {
                if (i >= imbalance_start_message_idx && i < imbalance_start_message_idx + imbalance_duration_messages) {
                    current_side_bias = imbalance_side_bias; // Bias towards one side (e.g., buy)
                    current_max_qty = max_qty * 5; // Larger quantities
                }
                break;
            }
            case MarketCondition::LiquidityDrain: {
                if (i >= drain_start_message_idx && i < drain_start_message_idx + drain_duration_messages) {
        ...
        if (action_type < current_add_prob) { // Add a new order
            msg.action = static_cast<databento::action::Action>('A'); // Corrected: static_cast to enum
            msg.order_id = next_order_id++;
            msg.size = random_quantity(gen, current_min_qty, current_max_qty);
            msg.side = (side_dist(gen) == 0) ? static_cast<databento::side::Side>('B') : static_cast<databento::side::Side>('A'); // Corrected: static_cast to enum

            if (condition == MarketCondition::QuoteStuffing) {
                if (msg.side == static_cast<databento::side::Side>('B')) {
                    msg.price = current_mid_price - quote_stuffing_price_offset - random_price(gen, 1, 10) * price_tick;
                } else {
                    msg.price = current_mid_price + quote_stuffing_price_offset + random_price(gen, 1, 10) * price_tick;
                }
            } else if (msg.side == static_cast<databento::side::Side>('B')) { // Corrected: static_cast to enum
                msg.price = current_mid_price + random_price(gen, 1, 5) * price_tick; // Aggressive buy: price above current mid (to hit asks)
            } else {
        ...
        } else if (action_type < current_add_prob + current_cancel_prob && !active_orders.empty()) { // Cancel an existing order
            msg.action = static_cast<databento::action::Action>('C'); // Corrected: static_cast to enum
            auto it = active_orders.begin();
            std::advance(it, std::uniform_int_distribution<size_t>(0, active_orders.size() - 1)(gen));
            msg.order_id = it->first;
            msg.price = it->second.price;
            msg.side = it->second.side;
            msg.size = it->second.size; // Size is not strictly needed for cancel, but good to populate
            active_orders.erase(it);
        } else if (action_type < current_add_prob + current_cancel_prob + current_modify_prob && !active_orders.empty()) { // Modify an existing order
            msg.action = static_cast<databento::action::Action>('M'); // Corrected: static_cast to enum
            auto it = active_orders.begin();
            std::advance(it, std::uniform_int_distribution<size_t>(0, active_orders.size() - 1)(gen));
            msg.order_id = it->first;
            msg.side = it->second.side;
            msg.price = it->second.price; // Keep same price for simplicity, or randomly change
            msg.size = random_quantity(gen, current_min_qty, current_max_qty); // New random size
            active_orders[msg.order_id] = msg; // Update active order map
        } else if (action_type < current_add_prob + current_cancel_prob + current_modify_prob + current_aggressive_prob) { // Aggressive Trading (Add that crosses the spread)
            msg.action = static_cast<databento::action::Action>('A'); // Aggressive orders are still 'Add' messages
            msg.order_id = next_order_id++;
            msg.size = random_quantity(gen, current_max_qty * 2, current_max_qty * 5); // Larger aggressive quantity
            
            // Bias side for LargeOrderImbalance
            if (condition == MarketCondition::LargeOrderImbalance && i >= imbalance_start_message_idx && i < imbalance_start_message_idx + imbalance_duration_messages) {
                msg.side = (std::uniform_int_distribution<>(0, 99)(gen) < current_side_bias) ? static_cast<databento::side::Side>('B') : static_cast<databento::side::Side>('A');
            } else {
                msg.side = (side_dist(gen) == 0) ? static_cast<databento::side::Side>('B') : static_cast<databento::side::Side>('A'); // Corrected: static_cast to enum
            }

            if (msg.side == static_cast<databento::side::Side>('B')) { // Corrected: static_cast to enum
            ...
            // Aggressive orders might not stay on the book, so don't add to active_orders map
        }

        encoder.EncodeRecord(msg); // Pass only the record

        // Simulate mid-price movement for volatility
        if (condition != MarketCondition::FlashCrash) { // FlashCrash handles its own price movement
            price_move_dist_gen.param(std::uniform_int_distribution<int>::param_type(current_price_move_min, current_price_move_max));
            current_mid_price += price_move_dist_gen(gen) * price_tick;
        }
        if (current_mid_price < initial_mid_price / 2) current_mid_price = initial_mid_price / 2;
        if (current_mid_price > initial_mid_price * 2) current_mid_price = initial_mid_price * 2;
    }

    std::cout << "Generated " << num_messages << " messages to " << output_filename << std::endl;
}
